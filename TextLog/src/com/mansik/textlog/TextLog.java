...
public class TextLog extends Activity {
	...
	// mPath는 SD 카드의 루트로 기본 초기화한다. SD 카드가 없으면 빈 문자열이다.
	//주의!! sd 카드이므로 manifest에서  READ_EXTERNAL_STORAGE, WRITE_EXTERNAL_STORAGE user permission 을 잡아줘야 된다.
	static {
		boolean HaveSD = Environment.getExternalStorageState().equals(
	...
	public static void init(Context main) {
		...
		if (mMaxFileSize != 0 && (mWhere & LOG_FILE) != 0) {
			...
			if (file.length() > mMaxFileSize * 1024) {
				...
				// 앞쪽 90%를 잘라낸다.
				log = log.substring(log.length() * 90 / 100);
	...
	static String getNowTime() {
		...
		String Time = String.format("%d-%d %d:%d:%d",
	...
	public static void o(String text, Object... args) {
		...
		// 예외의 getMessage가 null을 리턴하는 경우가 있어 널 점검이 필요하다.
		if (text == null) {
			return;
		}
	...
	public static void lap(String text) {
		...
		String sText = String.format("St=%4d,gap=%4d" + text, now - mStartTime,
				now - mLastTime);
		mLastTime = now;
		o(sText);
	}
	...
	public static void ViewLog() {
		...
		new AlertDialog.Builder(mMain).setTitle("Log").setView(scroll)
